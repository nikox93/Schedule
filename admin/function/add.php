...
<body>
	<div id="updating">
		<h1 align="center">Добавление новой записи</h1>
		...
		teach.last_name, gr.gr_name	FROM rasp, sub, type, gr, teach WHERE rasp.id_sub = sub.id_sub 
		AND rasp.id_type = type.id_type AND rasp.id_gr = gr.id_gr AND rasp.id_teach = teach.id_teach 
		AND rasp.id_gr = gr.id_gr";
	...
	2 => "Еженедельно"
			...
			<table align="center" border=1px>
				...
				<tr>
					...
					<td><input type="text" name="s_date" id="datepicker_1" autocomplete="off"></td>
				</tr>
				
				<tr>
					<td>Выберите дату окончания занятия</td>					
					<td><input type="text" name="e_date" id="datepicker_2" autocomplete="off"></td>
				</tr>';
			?>
				<tr>
					<td colspan="3">
						<p align="center">
							...
							<input type="submit" name="adding" id="adding"/>
						</p>
					</td>
				</tr>
			...
			<?php
				...
				if(isset($_POST['adding'])){
						...
						if(empty($s_date)) $s_date = date('Y-m-d', strtotime($_POST['s_date'])); // если дата не менялась - оставляем прежнюю 
						else $s_date = $_POST['s_date']; 					
						

						$e_date = trim($_POST['e_date']);
						if(empty($e_date)) $e_date = date('Y-m-d', strtotime($_POST['e_date'])); // та же песня, что и выше 
						else $e_date = $_POST['e_date']; 
												
						$query = "INSERT INTO rasp (day, lesson, id_week, id_teach, id_sub, id_cr, id_gr, id_type,
						s_date, e_date) VALUES ('$day','$lesson',$week,'$teach','$sub','$cr','$gr','$type','$s_date','$e_date')";
					
						$result = mysql_query($query) or die(mysql_error());
																	
						echo "<script>alert('Запись добавлена в БД. Нажмите ОК для продолжения'); 					
