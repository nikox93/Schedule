...
class SelectListTeacher
{
		...
		public function ShowKafTeach()
        {
            ...
            while($row = mysql_fetch_array($res))
            {
                $kaf .= '<option value="' . $row['id_kaf'] . '" title="' . $row['f_name_kaf'] . '">' . $row['s_name_kaf'] . '</option>';
            }
            return $kaf;
        }
		...
		public function ShowTeachTeach()
        {
            ...
            while($row = mysql_fetch_array($res))
            {
                $teach .= '<option value="' . $row['id_teach'] . '" 
				title="' . $row['last_name'] . ' ' .  $row['first_name'] . ' ' . $row['father_name'] . '">' 
				. $row['last_name'] . ' ' .  substr($row['first_name'],0,2) . '.' . substr($row['father_name'],0,2) .  
				'</option>';
            }
            return $teach;
        }		
...
$header = array(
  ...
  $end_date_sem = strtotime($semestr_date_end); // дата конца семестра config.php
		...
		/* ---------- Формирование запроса ----------------- */
			$query = "SELECT 
			rasp.id_gr, rasp.day, rasp.lesson, rasp.id_week, sub.s_name_sub, sub.f_name_sub, classroom.id_cr, type.id_type, DATE_FORMAT(rasp.s_date, '%d.%m.%Y') AS start_date, DATE_FORMAT(rasp.e_date, '%d.%m.%Y') AS end_date, gr.gr_name
		FROM 
